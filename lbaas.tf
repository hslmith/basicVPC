...
/////////////////////////////////////////
// Add web servers to pool
/////////////////////////////////////////


resource "ibm_is_lb_pool_member" "webtier-lb-pool-member-zone1" {
  count          = "${ibm_is_instance.web-instance.count}"
  lb             = "${ibm_is_lb.webtier-lb.id}"
  pool           = "${element(split("/", ibm_is_lb_pool.webtier-lb-pool.id),1)}"
  port           = "80"
  target_address = "${element(ibm_is_instance.web-instance.*.primary_network_interface.0.primary_ipv4_address,count.index)}"
}
*/